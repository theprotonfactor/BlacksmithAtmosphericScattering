...
inline int inShadow(half occlusion)
{
	if (occlusion > 0)
		return 1;
	else
...
half VolundSampleScatterOcclusion(half2 pos) {
	...
	if(maskWeight == 4.f || maskWeight == 0.f) {
		...
		fWeights.x = max(0.001f, fWeights.x);
		fWeights.y = max(0.001f, fWeights.y);
...
half3 VolundApplyScatterOcclusion(half4 coords1, half3 coords2, half2 pos, half3 color) {
	...
	float dist = Linear01Depth(tex2D(_CameraDepthTexture, UVFromPos(pos)).r);
	//dist = min(1.f, dist * 0.5);
	...
	u_ShadowBias *= dist;
	u_ShadowBiasIndirect *= dist;
	return
		// color * coords1.a
		lerp(color, color * coords1.a, dist)
		+ coords1.rgb * min(1.f, occlusion + u_ShadowBias) + coords2.rgb * min(1.f, occlusion + u_ShadowBiasIndirect);

	/*return
		lerp (color, color * coords1.a, occlusion)
		+ ((coords1.rgb + coords2.rgb) * occlusion);*/
...
// Original vert/frag macros
	...
	#else
		#define VOLUND_TRANSFER_SCATTER(position, o) VolundTransferScatterOcclusion(position, o.scatterCoords1, o.scatterCoords2)
...
#define SURFACE_SCATTER_TRANSFER(position, o)	VolundTransferScatterOcclusion(position, o.scatterCoords1, o.scatterCoords2)
